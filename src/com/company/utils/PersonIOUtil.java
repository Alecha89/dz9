...
public class PersonIOUtil {
    ...
    public static List<Person> readFile(String fileName) throws IOException, EmptySourceFileException, TextException {

        List<Person> persons = new ArrayList<>();

        if (PersonIOUtil.isFileBlank(fileName)){
            throw new EmptySourceFileException("Пустой файл!");
        }
    ...
    private static Person toPerson(String fileLine) throws TextException {
        ...
        if (dataArray.length != 3) {
            throw new TextException("Ошибка!");
        }
